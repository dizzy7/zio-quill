...
trait Unliftables extends QuatUnliftable {
  ...
  implicit val astUnliftable: Unliftable[Ast] = Unliftable[Ast] {
    ...
    case q"$pack.Infix.apply(${ a: List[String] }, ${ b: List[Ast] }, ${ pure: Boolean }, ${ transparent: Boolean }, ${ quat: Quat })" => Infix(a, b, pure, transparent, quat)
  ...
  implicit val queryUnliftable: Unliftable[Query] = Unliftable[Query] {
    ...
    case q"$pack.GroupTo.apply(${ a: Ast }, ${ b: Ident }, ${ c: Ast }, ${ d: Ident }, ${ e: Ast })" => GroupTo(a, b, c, d, e)
